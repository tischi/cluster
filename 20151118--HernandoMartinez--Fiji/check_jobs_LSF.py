...
if __name__ == '__main__':
    ...
    try:
        ...
        parser = argparse.ArgumentParser(
                    description='Ceck CellProfiler LSF jobs.')
        ...
        if logdir==None:
            raise Exception("--logdir parameter must be specified.")
        if not os.path.isdir(logdir):
            raise Exception("The chosen directory does not exist.")
        
        # to-do: extract max memory used
        
        files = os.listdir(jobdir)
        totalNumberOfJobs = len(files) - 1 # -1 because of the jobarray file
        
        files = os.listdir(logdir)
        d = {'PIL':[], 'not_started':[], 'out_file_incomplete_start':[], 'out_file_incomplete_end':[], 'started':[], 'jvm_dir_not_found':[], 'output':[], 'toucherror':[], 'libjvm_not_found':[], 'libjvm_found':[], 'libjvmerror':[], 'successful':[], 'caterror': [], 'failed':[], 'withError':[], 'unfinished':[], 'jobnumbers':[]}
        ...
        for job in d['started']:  # todo: replace by looping from min to max: this allows for finding "not started"
            ...
            if os.path.isfile(jobOutPath):  # if this file exists it means the job finished somehow
                ...
                elif not 'CreateBatchFiles' in file(jobOutPath,"r").readlines()[-7]:
                    #print job,"CreateBatchFiles is not in the 7th last line"
                    if not job in d['failed']:                    
                      d['failed'].append(job)
                    d['out_file_incomplete_end'].append(job)                
                    
                if not 'CellProfiler running...' in outFileContent:
                    #print job,"CellProfiler running... is missing at output"
                    d['out_file_incomplete_start'].append(job)
        ...
        print "  failed (incl. PIL usage): "+str(len(d['failed'])) 
        ...
        if len(d['not_started'])>0:
            ...
            #    print job            
            print d['not_started']
            answer = raw_input('\n(Re)submit not started jobs (please ONLY if NOT STARTED for LONG time)? (y/n) ')
            print ""
            if answer.lower() in ('y', 'yes'):
        ...
        print("")
        print("for concatenation of the image.csv output tables execute (copy and paste) below command (you may have to adapt the --tabledir input directory):")
        print("")
        cmd = "python-2.7 /g/almf/software/CP2C/concat_cp_tables.py --table_ending image.csv --tabledir "
        ...
        print("")
        print("")
        print("for concatenation of the objects.csv output tables execute (copy and paste) below command (you may have to adapt the --tabledir input directory)")
        print("")
        cmd = "python-2.7 /g/almf/software/CP2C/concat_cp_tables.py --table_ending objects.csv --tabledir "
        print(cmd+tabledir) 
